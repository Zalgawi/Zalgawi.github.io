  ...
  >
    <div v-if="project" class="p-6 sm:p-8">
      <div class="mb-5 flex items-start justify-between gap-4">
        <div>
          <span
            class="mb-2 block font-mono text-[10.5px] tracking-[.14em] text-ice uppercase"
            >{{ project.tag }}</span
          >
          <h2
            id="modal-title"
            class="display wdth-110 m-0 text-[24px] tracking-tight"
          >
            {{ project.title }}
          </h2>
        </div>
      ...
      <div
        class="grid min-h-45 place-items-center rounded-xl border border-dashed border-line"
      >
        <p
          class="m-0 font-mono text-[11px] tracking-[.14em] text-muted uppercase"
        >
